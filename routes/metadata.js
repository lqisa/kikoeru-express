...
router.get('/works',
  ...
  async (req, res, next) => {
    ...
    try {
      const query = () => db.getWorksBy({ username })
...
router.get('/search/:keyword?', async (req, res, next) => {
  ...
  try {
    const query = () => db.getWorksByKeyWord({ keyword, username })
    const totalCount = await query().count('id as count')
...
router.get('/:field(circle|tag|va)s/:id/works',
  ...
  async (req, res, next) => {
    ...
    try {
      const query = () => db.getWorksBy({ id: req.params.id, field: req.params.field, username })
      const totalCount = await query().count('id as count')
