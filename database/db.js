...
const databaseExist = fs.existsSync(
  path.join(config.databaseFolderDir, 'db.sqlite3')
)
  ...
  knex.transaction(async (trx) => {
    ...
    if (options.includeVA || options.refreshAll) {
      ...
      for (const va of work.vas) {
        await trx.raw('INSERT OR IGNORE INTO t_va(id, name) VALUES (?, ?)', [
          va.id,
          va.name
        ])
        await trx.raw(
          'INSERT OR IGNORE INTO r_va_work(va_id, work_id) VALUES (?, ?)',
          [va.id, work.id]
        )
      }
    }
    if (options.includeTags || options.refreshAll) {
      ...
      for (const tag of work.tags) {
        await trx.raw('INSERT OR IGNORE INTO t_tag(id, name) VALUES (?, ?)', [
          tag.id,
          tag.name
        ])
        await trx.raw(
          'INSERT OR IGNORE INTO r_tag_work(tag_id, work_id) VALUES (?, ?)',
          [tag.id, work.id]
        )
      }
    }
...
const removeWork = async (id, trxProvider) => {
  ...
  const circle = await trx('t_work')
    .select('circle_id')
    .where('id', '=', id)
    .first()
  const tags = await trx('r_tag_work')
    .select('tag_id')
    .where('work_id', '=', id)
...
const getWorksByKeyWord = ({ keyword, username = 'admin' } = {}) => {
  ...
  const circleIdQuery = knex('t_circle')
    .select('id')
    .where('name', 'like', `%${keyword}%`)

  const tagIdQuery = knex('t_tag')
    .select('id')
    .where('name', 'like', `%${keyword}%`)
  const vaIdQuery = knex('t_va')
    .select('id')
    .where('name', 'like', `%${keyword}%`)
...
const updateUserReview = async (
  ...
  knex.transaction(async (trx) => {
    ...
    if (starOnly) {
      ...
      await trx.raw(
        'INSERT OR IGNORE INTO t_review (user_name, work_id, rating) VALUES (?, ?, ?);',
        [username, workid, rating]
      )
      ...
      await trx.raw(
        'INSERT OR IGNORE INTO t_review (user_name, work_id, progress) VALUES (?, ?, ?);',
        [username, workid, progress]
      )
    } else {
      await trx.raw(
